...
class AutoUpdater:
    ...
    def _monitor(self):
        while not self._stop_flag.is_set():
            ...
            try:
                ...
                if current_time.tm_min % self.config.UPDATE_RATE_MINUTES == 0:
                    self._check_for_updates()
                    time.sleep(60)
                else:
                    sleep_minutes = (
                        self.config.UPDATE_RATE_MINUTES
                        - current_time.tm_min % self.config.UPDATE_RATE_MINUTES
                    )
    ...
    def _check_for_updates(self):
        try:
            ...
            branch_name = self.config.UPDATE_BRANCH_NAME
